...
def prepare_cascaded_tanks_overlapping_val_data(
    val_length: int = 200,
...
def main() -> None:
    ...
    base_lr = 1e-3
    weight_decay = 0.0
    overlap_val_length = 200
    patience = 10000
    tail_start = 50
    checkpoint_path = "checkpoints/CT_fulltrain_curriculum/best_resdynet_CT_fulltrain_curriculum.pth"
    clip_grad_norm = 0.5
    gamma_decay = 0.98
    curriculum_horizons = [10, cfg.horizon]
    curriculum_epochs = [300, 300]
    curriculum_lrs = [1e-3, 2e-4]
    ...
    log_stage("Preparing Cascaded Tanks dataset")
    data = prepare_cascaded_tanks_overlapping_val_data(
    ...
    for stage_idx, (stage_horizon, stage_epochs, stage_lr) in enumerate(
        ...
        if stage_idx - 1 < resume_stage_idx:
            print(
                f"Skipping curriculum stage H={stage_horizon} "
                f"because resume starts at H={resume_from_horizon}.",
                flush=True,
            )
            continue
    ...
    print("RMSE:", test_eval_chunked["rmse"].numpy(), flush=True)
    ...
    plot_chunked_test_prediction(
        ...
        save_path="outputs/ct_chunked_test_prediction.png",
        title="ResDyNet - Cascaded Tanks chunked test prediction",
    )
